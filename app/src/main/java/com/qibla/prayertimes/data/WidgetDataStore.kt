...
class WidgetDataStore(context: Context) {
    ...
    fun save(cityName: String, timings: Map<String, String>, hijri: HijriDate?, isOffline: Boolean = false) {
        ...
        // the user's correction via its own `adjustment` parameter) when available, otherwise
        // the local offline calculation shifted the same way — so the widget always agrees
        // with the app's home screen regardless of which source was used.
